...
class GraphBuilder {
  ...
  _collectSourceFiles() {
    return collectSourceFiles(this.projectDir, {
      extensions: ['js', 'ts', 'jsx', 'tsx', 'py', 'go', 'rs', 'java', 'rb', 'php'],
      maxDepth: 6,
    });
  }
  ...
  _processFile(filePath, relPath) {
    ...
    const fileNodeId = this.graph.addNode(NODE_TYPES.FILE, basename, {
      file: relPath,
      summary: this._summarizeFileFromContent(content, ext),
    });
  ...
  /** Summarize a file from its already-read content (no extra I/O). */
  _summarizeFileFromContent(content, ext) {
    if (!content) return '';
